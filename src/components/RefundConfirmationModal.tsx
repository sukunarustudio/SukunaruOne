...
const COMMON_REASONS = [
  'Salah input kasir / salah harga',
  'Permintaan pembeli / retur barang',
  'Produk dibatalkan / tidak jadi beli',
  'Transaksi duplikat',
  'Metode bayar salah',
  'Lainnya',
];
...
export const RefundConfirmationModal: React.FC<RefundConfirmationModalProps> = ({
  ...
  const finalReason =
    selectedPreset === 'Lainnya'
      ? customReason.trim() || 'Pembatalan transaksi'
      : customReason.trim()
      ...
      <div className="w-full max-w-lg bg-white rounded-3xl border border-[#BFC9D1]/30 shadow-2xl p-5 sm:p-6 text-left my-auto relative animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="flex items-start gap-3.5 mb-4 pr-8">
          <div className="w-12 h-12 rounded-2xl bg-rose-50 text-rose-600 border border-rose-200/80 flex items-center justify-center shrink-0 shadow-xs">
            <ReceiptRefundIcon className="w-6 h-6 stroke-[2]" />
          </div>
          <div>
            ...
            <p className="text-xs text-[#898989] mt-0.5 leading-relaxed">
              Transaksi tidak akan dihapus, melainkan statusnya diubah menjadi{' '}
              <span className="font-bold text-rose-700">Dibatalkan</span>.
            </p>
          </div>
        </div>
        ...
        <div className="bg-[#EAEFEF]/60 border border-[#BFC9D1]/30 rounded-2xl p-3.5 mb-4 space-y-2">
          ...
          <div className="flex items-center justify-between text-[11px] text-[#898989]">
            <span>Pelanggan: <strong className="text-[#25343F]">{transaction.customerName}</strong></span>
            <span>{formatDate(transaction.date)}</span>
          </div>

          {/* Items snippet */}
          <div className="text-[11px] text-[#25343F] bg-white rounded-xl p-2 border border-[#BFC9D1]/20">
        ...
        <div className="grid grid-cols-3 gap-2 mb-4 text-center">
          <div className="p-2 rounded-xl bg-slate-50 border border-slate-100">
            ...
            <span className="text-[10px] font-bold text-slate-700 block">Stok &amp; BOM</span>
          ...
          <div className="p-2 rounded-xl bg-slate-50 border border-slate-100">
            ...
            <span className="text-[10px] font-bold text-slate-700 block">Saldo Kas</span>
            <span className="text-[9px] text-rose-700 font-extrabold">- Dibalikkan</span>
          </div>
          <div className="p-2 rounded-xl bg-slate-50 border border-slate-100">
            <ChartBarIcon className="w-4 h-4 text-blue-600 mx-auto mb-0.5" />
            <span className="text-[10px] font-bold text-slate-700 block">Laba Bersih</span>
            <span className="text-[9px] text-blue-700 font-extrabold">Dikoreksi</span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-end gap-2.5 pt-2 border-t border-slate-100">
          ...
          >
            {isSubmitting ? (
              ...
              <>
                <ReceiptRefundIcon className="w-4 h-4 stroke-[2]" />
